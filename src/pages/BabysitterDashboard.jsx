...
import { Calendar, CheckCircle, Heart, Clock } from "lucide-react";
...
export default function BabysitterDashboard() {
  ...
  const fetchReservations = async () => {
    try {
      ...
      const res = await axios.get(
        `http://localhost:4000/api/reservations/babysitter/${babysitterId}`,
  ...
  const updateStatut = async (id, statut) => {
    try {
      ...
      await axios.put(
        `http://localhost:4000/api/reservations/modifier/${id}`,
        { statut },
        { headers: { Authorization: `Bearer ${token}` } }
      );
    ...
    >
      <h1 className="text-2xl font-bold text-pink-600 cursor-pointer">
        ...
        <nav className="flex gap-6 text-gray-700 font-medium flex-wrap">
          <Link to="/babysitterDashboard" className="hover:text-pink-600">Tableau de bord</Link>
          <Link to="/search-results" className="hover:text-pink-600">Recherche</Link>
          <Link to="/messages" className="hover:text-pink-600">Messages</Link>
        </nav>
      ...
      <div className="bg-white rounded-xl shadow-sm p-6 mb-8 mx-4 sm:mx-6 lg:mx-8">
        ...
        <div className="space-y-4">
          {loading && <div className="text-center py-10 text-gray-500 animate-pulse">Loading...</div>}
          {!loading && prochaines.length === 0 && <div className="text-center py-10 text-gray-400">Aucune réservation.</div>}

          {prochaines.map((r) => (
            <div key={r._id} className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm hover:shadow-lg transition duration-300">
              <div className="flex justify-between items-center">
                {/* LEFT INFO */}
                <div className="space-y-2">
                  <h4 className="text-lg font-bold text-gray-800">{r.parentId?.nom || "Parent"}_{r.parentId?.prenom || ""}</h4>
                  ...
                  <div className="flex items-center gap-2 text-sm text-gray-600">
                    ...
                    {new Date(r.dateHeureDebut).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}
                  </div>
                  <span
              ...
              {r.statut}
                ...
                <div className="flex flex-col gap-2">
                  <button
                    ...
                    className="px-4 py-2 bg-white text-pink-600 border border-pink-500 rounded-lg hover:bg-pink-400 hover:text-white text-sm font-semibold transition" >                   
                    Confirmer
                  </button>
                  ...
                  <button
                    onClick={() => navigate(`/messages/${r.parentId?._id}`)}
      ...
      <div className="bg-white rounded-xl shadow-sm p-6 mb-8 mx-4 sm:mx-6 lg:mx-8">
        ...
        <div className="space-y-4">
          {!loading && passees.length === 0 && <div className="text-center py-10 text-gray-400">Aucune réservation passée.</div>}
